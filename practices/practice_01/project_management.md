...
| 2. Ограничение размера `diff` | Слишком большие `diff` отклоняются контролируемым 4xx; критерий «слишком большой» задаётся утверждённым человеком порогом (значение — Неизвестно / требует уточнения) | Утверждает значение порога (Неизвестно / требует уточнения) и тексты ошибок | Предлагает подход к измерению размера и тест-кейсы | Проверка (исполняется после утверждения порога): вход — `diff` размером больше утверждённого порога; ожидание — контролируемый 4xx | Инкремент 1 |
